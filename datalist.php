...
class DataList{
	...
	// handle user commands ...
		...
		elseif($delete_x != ''){
			...
			if($delete_res){
				...
				if($Embedded){
					$this->HTML .= '<script>$j(function(){ setTimeout(function(){ sporttracker.closeParentModal(); }, 2000); })</script>';
				}
			}
		}
	...
	function tv_tools(){
		...
		<div class="pull-right flip btn-group vspacer-md hspacer-md tv-tools">
			<button title="<?php echo html_attr($Translation['previous column']); ?>" type="button" class="btn btn-default tv-scroll" onclick="sporttracker.TVScroll().less()"><i class="glyphicon glyphicon-step-backward"></i></button>
			<button title="<?php echo html_attr($Translation['next column']); ?>" type="button" class="btn btn-default tv-scroll" onclick="sporttracker.TVScroll().more()"><i class="glyphicon glyphicon-step-forward"></i></button>
		</div>
		...
		<script>
			$j(function(){
				...
				var show_column = function(col_class, show){
					...
					sporttracker.TVScroll().reset();

					col_cookie(col_class, show);
				}

				/* initiate TVScroll */
				sporttracker.TVScroll().less();
