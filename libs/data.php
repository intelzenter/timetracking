...
function checkdetailsStudent(){
	...
	if ($group=="students") {
		...
		if ($countcheck==0) {
			# code...
			echo '<div class="alert alert-primary"><strong>Whats up '.$name.', Welcome to sporttracker-your timetable companion.You need to provide your personal details to be able to get personalized content.Simply click <a href="student_details_view.php">HERE</a> to get started</strong></div>';
		}
	}
}
...
function getclassestoday(){
	...
	$getclasses=mysqli_query($con,"SELECT * FROM practice_time_table WHERE day LIKE '$today%' AND school='$storeSchool' AND department='$storeDepartment' AND year_of_study='$storeYos'");
			#count check here
...
function getexams(){
	...
	$getexams=mysqli_query($con,"SELECT * FROM game_time_table WHERE date='$today' AND school='$storeSchool' AND department='$storeDepartment' AND year_of_study='$storeYos'");
			#count check here
...
function examttlink(){
	...
	echo'game_time_table_view.php?SortField=&SortDirection=&FilterAnd%5B1%5D=and&FilterField%5B1%5D=7&FilterOperator%5B1%5D=equal-to&FilterValue%5B1%5D='.$storeSName.'&FilterAnd%5B2%5D=and&FilterField%5B2%5D=8&FilterOperator%5B2%5D=equal-to&FilterValue%5B2%5D='.$storeDName.'&FilterAnd%5B3%5D=and&FilterField%5B3%5D=9&FilterOperator%5B3%5D=equal-to&FilterValue%5B3%5D='.$storeYos.'';
}
?>