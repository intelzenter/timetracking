		...
		$pcConfig = array(
			...
			'practice_time_table' => array(   
				'school' => array(   
					...
					'sortable-fields' => array(0 => '`practice_time_table`.`id`', 1 => 2, 2 => '`practice_time_table`.`time_start`', 3 => '`practice_time_table`.`time_end`', 4 => 5, 5 => 6, 6 => '`schools1`.`name`', 7 => '`departments1`.`name`', 8 => 9),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-practice_time_table',
					'template-printable' => 'children-practice_time_table-printable',
					'query' => "SELECT `practice_time_table`.`id` as 'id', `practice_time_table`.`day` as 'day', TIME_FORMAT(`practice_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`practice_time_table`.`time_end`, '%r') as 'time_end', `practice_time_table`.`unit_code` as 'unit_code', `practice_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`schools1`.`name`), CONCAT_WS('',   `schools1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `practice_time_table`.`year_of_study` as 'year_of_study' FROM `practice_time_table` LEFT JOIN `schools` as schools1 ON `schools1`.`id`=`practice_time_table`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`practice_time_table`.`department` "
				),
				'department' => array(   
			...
			'game_time_table' => array(   
				'school' => array(   
					...
					'sortable-fields' => array(0 => '`game_time_table`.`id`', 1 => '`game_time_table`.`date`', 2 => '`game_time_table`.`time_start`', 3 => '`game_time_table`.`time_end`', 4 => 5, 5 => 6, 6 => '`schools1`.`name`', 7 => '`departments1`.`name`', 8 => 9),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-game_time_table',
					'template-printable' => 'children-game_time_table-printable',
					'query' => "SELECT `game_time_table`.`id` as 'id', if(`game_time_table`.`date`,date_format(`game_time_table`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`game_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`game_time_table`.`time_end`, '%r') as 'time_end', `game_time_table`.`unit_code` as 'unit_code', `game_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`schools1`.`name`), CONCAT_WS('',   `schools1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `game_time_table`.`year_of_study` as 'year_of_study' FROM `game_time_table` LEFT JOIN `schools` as schools1 ON `schools1`.`id`=`game_time_table`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`game_time_table`.`department` "
				),
				'department' => array(   
					'parent-table' => 'departments',
					'parent-primary-key' => 'id',
