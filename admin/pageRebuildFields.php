<?php
	...
	/*
		$schema: [ tablename => [ fieldname => [ sporttracker => '...', 'db' => '...'], ... ], ... ]
	*/

	/* application schema as created in sporttracker */
	$schema = array(   
		'schools' => array(   
			'id' => array('sporttracker' => 'INT unsigned not null primary key auto_increment '),
			'name' => array('sporttracker' => 'VARCHAR(40) not null ')
		),
		'departments' => array(   
			'id' => array('sporttracker' => 'INT unsigned not null primary key auto_increment '),
			'name' => array('sporttracker' => 'VARCHAR(40) not null '),
			'school' => array('sporttracker' => 'INT unsigned not null ')
		),
		'practice_time_table' => array(   
			'id' => array('sporttracker' => 'INT unsigned not null primary key auto_increment '),
			'day' => array('sporttracker' => 'VARCHAR(40) not null '),
			'time_start' => array('sporttracker' => 'TIME not null '),
			'time_end' => array('sporttracker' => 'TIME not null '),
			'unit_code' => array('sporttracker' => 'VARCHAR(40) not null '),
			'venue' => array('sporttracker' => 'VARCHAR(40) not null '),
			'school' => array('sporttracker' => 'INT unsigned not null '),
			'department' => array('sporttracker' => 'INT unsigned not null '),
			'year_of_study' => array('sporttracker' => 'VARCHAR(40) not null ')
		),
		'game_time_table' => array(   
			'id' => array('sporttracker' => 'INT unsigned not null primary key auto_increment '),
			'date' => array('sporttracker' => 'DATE not null '),
			'time_start' => array('sporttracker' => 'TIME not null '),
			'time_end' => array('sporttracker' => 'TIME not null '),
			'unit_code' => array('sporttracker' => 'VARCHAR(40) not null '),
			'venue' => array('sporttracker' => 'VARCHAR(40) not null '),
		...
		'personal_time_table' => array(   
			...
			'time_start' => array('sporttracker' => 'TIME not null '),
			'time_end' => array('sporttracker' => 'TIME not null '),
			'activity' => array('sporttracker' => 'VARCHAR(40) not null ')
		),
		'student_details' => array(   
			'id' => array('sporttracker' => 'INT unsigned not null primary key auto_increment '),
			'full_name' => array('sporttracker' => 'VARCHAR(40) not null '),
			'school' => array('sporttracker' => 'INT unsigned not null '),
			'department' => array('sporttracker' => 'INT unsigned not null '),
			'year_of_study' => array('sporttracker' => 'VARCHAR(40) not null '),
			'reg_no' => array('sporttracker' => 'VARCHAR(40) not null unique ')
		),
		'notices' => array(   
			'id' => array('sporttracker' => 'INT unsigned not null primary key auto_increment '),
			'notice' => array('sporttracker' => 'TEXT not null '),
			'school' => array('sporttracker' => 'INT unsigned not null '),
			'department' => array('sporttracker' => 'INT unsigned not null '),
			'year_of_study' => array('sporttracker' => 'VARCHAR(40) not null '),
			'date' => array('sporttracker' => 'DATE ')
		)
	);
	...
	function fix_field($fix_table, $fix_field, $schema, &$qry){
		...
		if($row = db_fetch_assoc($res)){
			// modify field
			$qry = "alter table `{$fix_table}` modify `{$fix_field}` {$def['sporttracker']}";
			sql($qry, $eo);

			// remove unique from db if necessary
			if($row['Key'] == 'UNI' && !stripos($def['sporttracker'], ' unique')){
		...
		// create field
		$qry = "alter table `{$fix_table}` add column `{$fix_field}` {$schema[$fix_table][$fix_field]['sporttracker']}";
		sql($qry, $eo);
		return 2;
	}
	...
	foreach($table_captions as $tn => $tc){
		...
		if($res){
			while($row = db_fetch_assoc($res)){
				if(!isset($schema[$tn][$row['Field']]['sporttracker'])) continue;
	...
	if($fix_all){
		foreach($schema as $tn => $fields){
			foreach($fields as $fn => $fd){
				if(prepare_def($fd['sporttracker']) == prepare_def($fd['db'])) continue;
...
<table class="table table-responsive table-hover table-striped">
	<thead><tr>
		...
		<th><?php echo $Translation['sporttracker definition'] ; ?></th>
	...
	<?php foreach($schema as $tn => $fields){ ?>
		...
		<?php foreach($fields as $fn => $fd){ ?>
			<?php $diff = ((prepare_def($fd['sporttracker']) == prepare_def($fd['db'])) ? false : true); ?>
			...
			<tr class="<?php echo ($diff ? 'warning' : 'field_ok'); ?>">
				...
				<td><?php echo $fn; ?></td>
				<td class="<?php echo ($diff ? 'bold text-success' : ''); ?>"><?php echo $fd['sporttracker']; ?></td>
