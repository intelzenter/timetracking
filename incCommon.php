	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'practice_time_table' => array('Practice Time Table', '', 'resources/table_icons/blackboard_drawing.png', 'None'),
			'game_time_table' => array('Games Time Table', '', 'resources/table_icons/books.png', 'None'),
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'practice_time_table' => "`practice_time_table`.`id` as 'id', `practice_time_table`.`day` as 'day', TIME_FORMAT(`practice_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`practice_time_table`.`time_end`, '%r') as 'time_end', `practice_time_table`.`unit_code` as 'unit_code', `practice_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`schools1`.`name`), CONCAT_WS('',   `schools1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `practice_time_table`.`year_of_study` as 'year_of_study'",
			'game_time_table' => "`game_time_table`.`id` as 'id', if(`game_time_table`.`date`,date_format(`game_time_table`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`game_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`game_time_table`.`time_end`, '%r') as 'time_end', `game_time_table`.`unit_code` as 'unit_code', `game_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`schools1`.`name`), CONCAT_WS('',   `schools1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `game_time_table`.`year_of_study` as 'year_of_study'",
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'practice_time_table' => "`practice_time_table` LEFT JOIN `schools` as schools1 ON `schools1`.`id`=`practice_time_table`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`practice_time_table`.`department` ",
			'game_time_table' => "`game_time_table` LEFT JOIN `schools` as schools1 ON `schools1`.`id`=`game_time_table`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`game_time_table`.`department` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('sporttracker_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('sporttracker_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['sporttracker_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['sporttracker_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<!-- application title is obtained from the name besides the yellow database icon in sporttracker, use underscores for spaces -->
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> sporttracker</a>
			</div>
